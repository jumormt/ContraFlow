...
from src.sequence_analyzer.scoring_system import ScoringSystem
'''
Authors:
...
class SequencesAnalyzer:
    ...
    def global_alignment(self) -> Tuple[str, str]:
        result: Dict[str,
                     Any] = self.needleman_wunsch_algorithm(minimize=False,
                                                            alignment_cal=True)
    ...
    def edit_distance(self) -> int:
        ...
        print(f"[Edit distance] Cost={result['score']}\n"
              f"{result['result_matrix']}\n"
              f"{result['traceback_matrix']}\n")
        return result['score']

    def needleman_wunsch_algorithm(
            self,
            minimize: bool = False,
            alignment_cal: bool = False) -> Dict[str, Any]:
        '''
        ...
        for row in range(1, rows):
            for col in range(1, cols):
                ...
                leave_or_replace_letter = H[row - 1, col - 1] + score_func(
                    a, b)
        ...
        return {
            ...
            'score': H[-1, -1] /
            (len(self.seq_a) + len(self.seq_b)),  # Always right-bottom corner
    ...
    def smith_waterman_algorithm(self) -> Dict[str, Any]:
        ...
        for row in range(1, rows):
            for col in range(1, cols):
                ...
                scores = [
                    leave_or_replace_letter, delete_indel, insert_indel, 0
                ]
                best_action = np.argmax(scores)
    ...
    def _traceback(self, result_matrix, traceback_matrix,
                   start_pos: Tuple[int, int],
                   global_alignment: bool) -> Tuple[str, str]:
        seq_a_aligned = ''
        seq_b_aligned = ''
        ...
        if global_alignment:
            ...
            end_condition_reached = lambda row, col: result_matrix[row, col
                                                                   ] == 0
